...
async def test_init_loop_success(ini_file):
    ...
    class Svc(ProcessBase):
        def __init__(self): super().__init__("a", "1.0")
        async def start_dbus(self): self.bus = MockBus(); return True
    svc = Svc()
...
async def test_init_loop_failure(ini_file):
    from pympacds.process import ProcessBase
    class Svc(ProcessBase):
        def __init__(self): super().__init__("b", "1.0")
...
async def test_close_loop_with_pending(ini_file):
    from pympacds.process import ProcessBase
    class Svc(ProcessBase):
        def __init__(self): super().__init__("c", "1.0")
        async def start_dbus(self): self.bus = MockBus(); return True
    svc = Svc()
    svc.setup(["-c", ini_file])
    assert await svc.init_loop()
...
async def test_main_loop_runs(ini_file):
    from pympacds.process import ProcessBase
    class Svc(ProcessBase):
        def __init__(self): super().__init__("d", "1.0")
        async def start_dbus(self): self.bus = MockBus(); return True
...
async def test_main_loop_init_fails(ini_file):
    from pympacds.process import ProcessBase
    class Svc(ProcessBase):
        def __init__(self): super().__init__("e", "1.0")
        async def start_dbus(self): return False
    svc = Svc()
    svc.setup(["-c", ini_file])
    await asyncio.wait_for(svc.main_loop(), timeout=2)
...
def test_signal_handler_no_event(process_base):
    ...
    async def _run(): await process_base._signal_term_handler(15)
    asyncio.run(_run())
...
async def test_middleware_setup_continue(ini_file):
    ...
    class TMW(MiddlewareBase):
        async def setup(self): raise ValueError("x")
        def on_error(self, e): return True
    class Svc(ProcessBase):
        def __init__(self): super().__init__("mw", "1.0")
        async def start_dbus(self): self.bus = MockBus(); return True
    svc = Svc()
    svc.setup(["-c", ini_file])
...
async def test_middleware_setup_abort(ini_file):
    ...
    class SMW(MiddlewareBase):
        async def setup(self): raise ValueError("x")
    class Svc(ProcessBase):
        def __init__(self): super().__init__("mw2", "1.0")
        async def start_dbus(self): self.bus = MockBus(); return True
    svc = Svc()
    svc.setup(["-c", ini_file])
...
async def test_middleware_teardown(ini_file):
    ...
    class TDMW(MiddlewareBase):
        async def teardown(self): calls.append(1)
    class Svc(ProcessBase):
        def __init__(self): super().__init__("mw3", "1.0")
        async def start_dbus(self): self.bus = MockBus(); return True
    svc = Svc()
    svc.setup(["-c", ini_file])
    svc._middleware_instances = [TDMW(svc, None)]
    assert await svc.init_loop()
    svc.exitevent.set()
    await svc.close_loop()
    assert len(calls) == 1
...
def test_validation_int_max(ini_file, tmp_path):
    ...
    cp["s"] = {"n": "10"}
    with open(ini_file, "w") as f: cp.write(f)
    p = ProcessBase("t", "1.0")
...
def test_logfile_fallback_tmp(ini_file, monkeypatch):
    ...
    def fail_init(self, fn, *a, **kw):
        if "/root/" in fn: raise OSError("nope")
        orig(self, fn, *a, **kw)
...
def test_cli_config_validate_with_schema(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, "argv", ["pympacds-admin", "config", "validate", "-c", str(cfg), "-s", str(schema)])
    from pympacds.cli import main
    main()
...
def test_cli_config_get_missing(monkeypatch, tmp_path):
    cfg = tmp_path / "t.ini"
    cfg.write_text("[DEFAULT]\nloglevel = WARN\nlogstdout = false\nlogfile =\n[x]\ny = z\n")
    monkeypatch.setattr(sys, "argv", ["pympacds-admin", "config", "get", "-c", str(cfg), "x", "MISSING"])
    from pympacds.cli import main
...
def test_cli_setup_warning(monkeypatch):
    ...
    class Args:
        resource_dir = "/x"; bus_prefix = "t"; no_dirs = False; dry_run = False; quiet = False
    _cmd_setup(Args)
...
def test_cli_setup_quiet(monkeypatch):
    ...
    monkeypatch.setattr(os.path, "exists", lambda p: False)
    class Args:
        resource_dir = "/x"; bus_prefix = "t"; no_dirs = False; dry_run = False; quiet = True
    _cmd_setup(Args)
...
def test_cli_setup_dry_run_template(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(os, "makedirs", lambda *a, **kw: None)
    class Args:
        resource_dir = str(tmp_path); bus_prefix = "t"; no_dirs = True; dry_run = True; quiet = True
    _cmd_setup(Args)
...
def test_cli_setup_update(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(os, "makedirs", lambda *a, **kw: None)
    class Args:
        resource_dir = str(tmp_path); bus_prefix = "com.new"; no_dirs = True; dry_run = True; quiet = True
    _cmd_setup(Args)


def test_cli_install_dry(monkeypatch, tmp_path):
    from pympacds.cli import _cmd_install
    d = tmp_path / "share"; d.mkdir()
...
def test_cli_install_full(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("pympacds.cli._find_resource", staticmethod(lambda fn: str(d / fn)))
    calls = []
    monkeypatch.setattr("pympacds.cli._run", lambda *a, **kw: calls.append(a[0]) or type("R", (), {"returncode": 0})())
    monkeypatch.setattr("pympacds.cli.shutil.copy", lambda *a: None)
    monkeypatch.setattr("pympacds.cli.os.path.exists", lambda p: False)
    class Args:
        service_name = "svc"; dry_run = False; no_enable = False; start = True
    _cmd_install(Args)
    assert len(calls) >= 3
